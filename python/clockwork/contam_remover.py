...
class ContamRemover:
    '''Takes a BAM file in read name order (ie has not been sorted by reference).
       ...
       3. Unmapped, where both reads unmapped. By default, these reads are included
          ...
          files instead'''
    def __init__(self, metadata_file, bam_file, counts_outfile, reads_out_1, reads_out_2, contam_out_1=None, contam_out_2=None, no_match_out_1=None, no_match_out_2=None, done_file=None):
        ...
        assert contam_out_1 == None == contam_out_2 or None not in {contam_out_1, contam_out_2}
        assert no_match_out_1 == None == no_match_out_2 or None not in {no_match_out_1, no_match_out_2}
    ...
    def _load_metadata_file(cls, infile):
        ...
        with open(infile) as f:
            for line in f:
                ...
                if group in data_by_group and data_by_group[group]['contam'] != is_contam:
                    raise Error('Error! is_contam for one group must always be the same (column 2). Check group "' + group + '"')
    ...
    def _sam_to_fastq(sam):
        ...
        seq = pyfastaq.sequences.Fastq(name, utils.decode(sam.seq), utils.decode(sam.qual))
    ...
    def _write_read_counts_by_group_file(cls, sequences_by_group, read_counts, outfile):
        ...
        for group in sequences_by_group:
            group_counts[group] = sum([read_counts.get(sequence, 0) for sequence in sequences_by_group[group]['sequences']])
        ...
        with open(outfile, 'w') as f:
            ...
            print('Reads_kept_after_remove_contam', 0, read_counts.get('reads_kept_after_remove_contam', 0), sep='\t', file=f)


    def run(self):
        ref_seqs_by_group, ref_seqs_contam = ContamRemover._load_metadata_file(self.metadata_file)
        filehandles = {f: pyfastaq.utils.open_file_write(self.files[f]) for f in self.files}
        ...
        for current_sam in sam_reader.fetch(until_eof=True):
            ...
            read_1_mapped, read_1_wanted = ContamRemover._read_mapped_and_wanted(previous_sam, ref_seqs_contam)
            read_2_mapped, read_2_wanted = ContamRemover._read_mapped_and_wanted(current_sam, ref_seqs_contam)
            ...
            if out_type is not None:
                print(ContamRemover._sam_to_fastq(previous_sam), file=filehandles[out_type + '1'])
                print(ContamRemover._sam_to_fastq(current_sam), file=filehandles[out_type + '2'])
        ...
        if previous_sam is not None:
            raise Error('Odd number of sequences in input BAM. Expected paired reads only.')

        ContamRemover._write_read_counts_by_group_file(ref_seqs_by_group, read_counts, self.counts_outfile)
