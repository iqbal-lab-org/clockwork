...
class ReferenceDir:
    def __init__(self, pipeline_references_root_dir=None, reference_id=None, directory=None):
        ...
        if self.directory is not None:
            ...
            self.pipeline_references_root_dir = os.path.abspath(pipeline_references_root_dir)
            self.directory = os.path.join(self.pipeline_references_root_dir, str(self.reference_id))
        else:
            raise Error('Must provide directory, or both of pipeline_references_root_dir,reference_id')
        ...
        self.remove_contam_metadata_tsv = os.path.join(self.directory, 'remove_contam_metadata.tsv')
    ...
    def make_index_files(self, fasta_in, genome_is_big, using_cortex, cortex_mem_height=22):
        ...
        if using_cortex:
            cortex.make_run_calls_index_files(self.ref_fasta, self.ref_fasta_prefix, mem_height=cortex_mem_height)


    def add_remove_contam_metadata_tsv(self, infile):
        utils.rsync_and_md5(infile, self.remove_contam_metadata_tsv)
        data_by_group, sequence_is_contam = contam_remover.ContamRemover._load_metadata_file(self.remove_contam_metadata_tsv)
