...
class ReadPairImporter:
    ...
    @classmethod
    def _check_database(
        cls, database, seqrep_id, isolate_id, sequence_replicate_number
    ):
        rows = database.get_rows_from_table(
    ...
    def _update_database(
        cls, database, seqrep_id, isolate_id, sequence_replicate_number, import_status=1
    ):
        ReadPairImporter._check_database(
            database, seqrep_id, isolate_id, sequence_replicate_number
        )
    ...
    def run(self):
        ReadPairImporter._check_database(
            self.db, self.seqrep_id, self.isolate_id, self.sequence_replicate_number
        )
        ...
        iso_dir = isolate_dir.IsolateDir(
            self.pipeline_root_dir, self.sample_id, self.isolate_id
        )
        ...
        try:
            ...
            ReadPairImporter._copy_reads_file(
                self.reads_file_1,
                iso_dir.reads_filename("original", self.sequence_replicate_number, 1),
                self.reads_file_md5_1,
            )
            ReadPairImporter._copy_reads_file(
                self.reads_file_2,
                iso_dir.reads_filename("original", self.sequence_replicate_number, 2),
                self.reads_file_md5_2,
            )
            ReadPairImporter._update_database(
                self.db,
                self.seqrep_id,
                self.isolate_id,
