...
class DbConnection:
    def __init__(self, ini_file, create=False, destroy=False, must_exist=False):
        ...
        if destroy:
            if must_exist:
                ...
                self.connection.cursor().execute('DROP DATABASE IF EXISTS ' + login_info['db'])
    ...
    def _parse_config_file(cls, ini_file):
        ...
        if len(missing_keys):
            raise Error('Error! Missing these keys from [db_login] section: ' + ','.join(list(missing_keys)))

        login_config = dict(config['db_login'])
        login_config['port'] = int(login_config['port']) if 'port' in login_config else None
        return login_config
