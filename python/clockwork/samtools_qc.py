...
class SamtoolsQc:
            ...
            for line in f:
                ...
                if line.startswith('SN'):
                    ...
                    if fields[1] in wanted_keys:
                        key = fields[1].replace(' ', '_').rstrip(':').replace('(', '').replace(')', '')
                        value = float(fields[2]) if wanted_keys[fields[1]] else int(fields[2])
    ...
    def run(self):
        ...
        hsc = het_snp_caller.HetSnpCaller(samfile, self.ref_fasta, os.path.join(self.outdir, 'het_snps'))
        hsc.run()
