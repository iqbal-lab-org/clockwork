...
class Fastqc:
    ...
    def _stats_from_report(filename):
        ...
        with open(filename) as f:
            for line in f:
                if (line.startswith('>>') and not line.startswith('>>END_MODULE')) or line.split('\t')[0] in interesting_line_starts:
                    try:
                    ...
                    if stat == 'sequence_length':
                        stats['min_sequence_length'], stats['max_sequence_length'] = Fastqc._seq_length_from_report_string(value)
                    else:
                        try:
                            value = int(value)
                        except:
    ...
    def _clean_outdir(outdir):
        ...
        dirs = [os.path.join(outdir, x) for x in os.listdir(outdir) if os.path.isdir(os.path.join(outdir, x))]
        ...
        for directory in dirs:
            for filename in [directory + '.zip', directory + '.html']:
                try:
                    ...
                    raise Error('Error deleting file "' + filename + '". Cannot continue')
