...
class IsolateDir:
    def __init__(self, root_dir, sample_id, isolate_id):
        ...
        self.sample_dir = IsolateDir._make_sample_dir_path(
            self.root_dir, self.sample_id
        )
    ...
    def contamination_counts_filename(self, sequence_replicate):
        return os.path.join(
            self.reads_dir,
            "reads.remove_contam." + str(sequence_replicate) + ".counts.tsv",
        )
    ...
    def pipeline_dir(
        self, sequence_replicate, pipeline_name, pipeline_version, reference_id=None
    ):
        final_dir = (
            pipeline_version
            if reference_id is None
            else pipeline_version + ".ref." + str(reference_id)
        )
        return os.path.join(
            self.pipelines_dir, str(sequence_replicate), pipeline_name, final_dir
        )
