...
def _make_dummy_success_receipt(outfile, object_type):
    accession = ''.join([random.choice(string.ascii_uppercase + string.digits) for _ in range(10)])
    with open(outfile, 'w') as f:
        print(r'''<?xml version="1.0" encoding="UTF-8"?>
<?xml-stylesheet type="text/xsl" href="receipt.xsl"?>
<RECEIPT receiptDate="2017-08-31T11:07:50.251+01:00" submissionFile="submission.xml" success="true">
    <''' + object_type.upper() + r''' accession="''' + accession + r'''" alias="alias" status="PRIVATE">
...
def parse_config_file(ini_file):
    ...
    if not ('user' in config['ena_login'] and 'password' in config['ena_login']):
        raise Error('Error! user and password not found in [ena_login] section of config file ' + ini_file)

    return config['ena_login']['user'], config['ena_login']['password']


def submit_xml_files(ini_file, outfile, files=None, use_test_server=False, unit_test=None, unit_test_obj_type=None):
    username, password = parse_config_file(ini_file)
    if files is None:
        files_command = None
    else:
        files_command_list = ['-F "' + key + '=@' + value + '"' for key, value in files.items()]
