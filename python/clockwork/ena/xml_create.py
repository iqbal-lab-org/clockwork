...
def make_add_submission_xml(outfile, submission_alias, center_name, source_xml, schema, broker_name=None, hold_for_two_years=True):
    if broker_name is None:
        submission = ET.Element('SUBMISSION', {'alias': submission_alias, 'center_name': center_name})
    else:
        submission = ET.Element('SUBMISSION', {'alias': submission_alias, 'center_name': center_name, 'broker_name': broker_name})
    ...
    if hold_for_two_years:
        ...
        hold = ET.SubElement(action_hold, 'HOLD', {'HoldUntilDate': two_years_from_today.isoformat()})
...
def make_project_xml(outfile, alias, center_name, title_text, description_text):
    ...
    project = ET.SubElement(project_set, 'PROJECT', {'alias': alias, 'center_name': center_name})
...
def make_sample_xml(outfile, sample_alias, center_name, title, taxon_id, sample_attributes_dict=None):
    sample_set = ET.Element('SAMPLE_SET')
    sample = ET.SubElement(sample_set, 'SAMPLE', {'alias': sample_alias, 'center_name': center_name})
    ...
    if sample_attributes_dict is not None:
        ...
        for tag, value in sorted(sample_attributes_dict.items()):
            sample_attribute = ET.SubElement(sample_attributes_element, 'SAMPLE_ATTRIBUTE')
...
def make_experiment_xml(outfile, experiment_alias, center_name, title, study_accession, sample_accession, library_name, platform, instrument, experiment_attributes_dict=None):
    experiment_set = ET.Element('EXPERIMENT_SET')
    experiment = ET.SubElement(experiment_set, 'EXPERIMENT', {'alias': experiment_alias, 'center_name': center_name})
    ...
    if experiment_attributes_dict is not None:
        experiment_attributes_element = ET.SubElement(experiment, 'EXPERIMENT_ATTRIBUTES')

        for tag, value in sorted(experiment_attributes_dict.items()):
            experiment_attribute = ET.SubElement(experiment_attributes_element, 'EXPERIMENT_ATTRIBUTE')
...
def make_paired_fastq_run_xml(outfile, run_alias, center_name, experiment_accession, file_1, md5_1, file_2, md5_2):
    run_set = ET.Element('RUN_SET')
    run = ET.SubElement(run_set, 'RUN', {'alias': run_alias, 'center_name': center_name})
    experiment_ref = ET.SubElement(run, 'EXPERIMENT_REF', {'accession': experiment_accession})
    data_block = ET.SubElement(run, 'DATA_BLOCK')
    files = ET.SubElement(data_block, 'FILES')
    file_1_data = { 'filename': file_1, 'filetype': 'fastq', 'checksum_method': 'MD5', 'checksum': md5_1}
    file_2_data = { 'filename': file_2, 'filetype': 'fastq', 'checksum_method': 'MD5', 'checksum': md5_2}
