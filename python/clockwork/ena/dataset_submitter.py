...
class DatasetSubmitter:
    ...
    def dataset_xml_file(cls, pipeline_root, dataset_name):
        return os.path.join(
            DatasetSubmitter.dataset_xml_dir(pipeline_root),
            dataset_name + ".dataset_submission.xml",
        )
    ...
    def _get_data_from_db(self):
        ...
        query = " ".join(
            ["SELECT", columns, "FROM", "(" + join + ")", "WHERE", "(" + where + ")"]
        )
    ...
    def _get_broker_name_from_ini_file(cls, ini_file):
        return DatasetSubmitter._get_key_from_ini_file(
            ini_file, "ena_login", "broker_name"
        )
    ...
    def _submit_runs(self, data_in):
        ...
        for row in data_in:
            ...
            fq_pairs_to_upload.append(
                (
                    ...
                    str(row["seqrep_id"])
                    + ".1."
                    + row["remove_contam_reads_file_1_md5"]
                    + ".fq.gz",
                    iso_dir.reads_filename(
                    ...
                    str(row["seqrep_id"])
                    + ".2."
                    + row["remove_contam_reads_file_2_md5"]
                    + ".fq.gz",
                )
            )
        ...
        upload_return_values = self.pool.starmap(
            ...
            zip(
                fq_pairs_to_upload,
                itertools.repeat(self.ini_file),
