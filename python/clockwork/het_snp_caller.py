...
class HetSnpCaller:
    def __init__(self, sorted_bam, ref_fasta, outprefix, min_total_depth=4, min_second_depth=2, max_allele_freq=0.9):
    ...
    @classmethod
    def _vcf_line_is_snp_and_or_het(cls, vcf_line, min_total_depth, min_second_depth, max_allele_freq):
        ...
        for i, adf in enumerate(adf_list):
            ...
            if adf >= min_second_depth and adf / adf_sum <= max_allele_freq and adr >= min_second_depth and adr / adr_sum <= max_allele_freq:
    ...
    @classmethod
    def _filter_vcf_and_count_snps(cls, vcf_file_in, vcf_file_out, min_total_depth, min_second_depth, max_allele_freq):
        results = {}
        ...
        with open(vcf_file_in) as f_in, open(vcf_file_out, 'w') as f_out:
            for line in f_in:
                ...
                chrom, is_snp, is_het = HetSnpCaller._vcf_line_is_snp_and_or_het(line, min_total_depth, min_second_depth, max_allele_freq)
    ...
    def _write_reports(cls, snp_data, contig_lengths, summary_out, per_contig_out):
        ...
        with open(per_contig_out, 'w') as f:
            print('Contig', 'Length', 'Positions_mapped', 'SNPs', 'Het_SNPs', sep='\t', file=f)
            ...
            for contig in sorted(contig_lengths):
                ...
                if contig in snp_data:
                    ...
                    print(contig, contig_lengths[contig], snp_data[contig]['positions'],
                            snp_data[contig]['snps'], snp_data[contig]['hets'],
                            sep='\t', file=f)
                else:
        ...
        with open(summary_out, 'w') as f:
            if totals['snps'] > 0:
                ...
                percent_snps = 0
            print('Total_length', 'Positions_used', 'Total_SNPs', 'Het_SNPs', 'Percent_SNPs_are_het', sep='\t', file=f)
            print(totals['length'], totals['positions'], totals['snps'], totals['hets'], percent_snps, sep='\t', file=f)
    ...
    def run(self):
        ...
        snp_data = HetSnpCaller._filter_vcf_and_count_snps(unfiltered_vcf, filtered_vcf,
            self.min_total_depth, self.min_second_depth, self.max_allele_freq)
