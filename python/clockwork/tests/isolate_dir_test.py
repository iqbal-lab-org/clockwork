...
class TestIsolateDir(unittest.TestCase):
    def test_sample_id_to_dir_numbers(self):
        ...
        self.assertEqual(['00', '00', '00', '00'], isolate_dir.IsolateDir.sample_id_to_dir_numbers(0))
        self.assertEqual(['00', '00', '00', '01'], isolate_dir.IsolateDir.sample_id_to_dir_numbers(1))
        self.assertEqual(['00', '00', '00', '99'], isolate_dir.IsolateDir.sample_id_to_dir_numbers(99))
        self.assertEqual(['00', '00', '01', '00'], isolate_dir.IsolateDir.sample_id_to_dir_numbers(100))
        self.assertEqual(['42', '00', '43', '00'], isolate_dir.IsolateDir.sample_id_to_dir_numbers(42004300))
        self.assertEqual(['12', '34', '56', '78'], isolate_dir.IsolateDir.sample_id_to_dir_numbers(12345678))
    ...
    def test_init(self):
        ...
        expected_sample_dir = isolate_dir.IsolateDir._make_sample_dir_path(tmp_root_dir, sample_id)
    ...
    def test_contamination_counts_filename(self):
        ...
        expected = os.path.join(iso_dir.isolate_dir, 'Reads', 'reads.remove_contam.1.counts.tsv')
        self.assertEqual(expected, iso_dir.contamination_counts_filename(1))
        expected = os.path.join(iso_dir.isolate_dir, 'Reads', 'reads.remove_contam.2.counts.tsv')
    ...
    def test_reads_filename(self):
        ...
        isolate_id = 42
        iso_dir = isolate_dir.IsolateDir(root, sample_id, isolate_id)
        self.assertEqual(os.path.join(iso_dir.isolate_dir, 'Reads', 'reads.original.11.1.fq.gz'), iso_dir.reads_filename('original', 11, 1))
        self.assertEqual(os.path.join(iso_dir.isolate_dir, 'Reads', 'reads.remove_contam.12.2.fq.gz'), iso_dir.reads_filename('remove_contam', 12, 2))
        self.assertEqual(os.path.join(iso_dir.isolate_dir, 'Reads', 'reads.contam.11.1.fq.gz'), iso_dir.reads_filename('contam', 11, 1))
    ...
    def test_pipeline_dir(self):
        ...
        isolate_id = 1
        iso_dir = isolate_dir.IsolateDir(root, sample_id, isolate_id)
        self.assertEqual(os.path.join(iso_dir.isolate_dir, 'Pipelines', '2', 'name', '1.0.0'), iso_dir.pipeline_dir(2, 'name', '1.0.0'))
        self.assertEqual(os.path.join(iso_dir.isolate_dir, 'Pipelines', '2', 'name', '1.0.1.ref.42'), iso_dir.pipeline_dir(2, 'name', '1.0.1', reference_id=42))
    ...
    def test_xml_submission_file(self):
        ...
        isolate_id = 42
        iso_dir = isolate_dir.IsolateDir(root, sample_id, isolate_id)
        self.assertEqual(os.path.join(iso_dir.sample_dir, 'ena_sample_submission.xml'), iso_dir.xml_submission_file('sample'))
        self.assertEqual(os.path.join(iso_dir.isolate_dir, 'ena_experiment_submission.xml'), iso_dir.xml_submission_file('experiment'))
        with self.assertRaises(isolate_dir.Error):
            iso_dir.xml_submission_file('run')
        self.assertEqual(os.path.join(iso_dir.reads_dir, 'reads.remove_contam.42.ena_run_submission.xml'), iso_dir.xml_submission_file('run', sequence_replicate=42))
