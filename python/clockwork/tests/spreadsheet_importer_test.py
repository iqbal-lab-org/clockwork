...
import os
import re
from clockwork import spreadsheet_importer, db, db_connection, db_maker, lock_file, utils
...
class TestSpreadsheetImporter(unittest.TestCase):
    ...
    def test_load_data_from_spreadsheet_tsv(self):
        ...
        got = spreadsheet_importer.SpreadsheetImporter.load_data_from_spreadsheet(filename)
        self.maxDiff = None
    ...
    def test_load_data_from_spreadsheet_bad_input(self):
        ...
        for filename in filenames:
            with self.assertRaises(spreadsheet_importer.Error):
                xlsx = os.path.join(data_dir, filename)
                spreadsheet_importer.SpreadsheetImporter.load_data_from_spreadsheet(xlsx)


    def test_looks_like_date(self):
        '''Test _looks_like_date'''
        self.assertFalse(spreadsheet_importer.SpreadsheetImporter._looks_like_date('not even close!'))
        self.assertFalse(spreadsheet_importer.SpreadsheetImporter._looks_like_date('20001-23-11'))
        self.assertFalse(spreadsheet_importer.SpreadsheetImporter._looks_like_date('2000-13-01'))
        self.assertFalse(spreadsheet_importer.SpreadsheetImporter._looks_like_date('2000-04-31'))
        self.assertFalse(spreadsheet_importer.SpreadsheetImporter._looks_like_date('20000430'))
        self.assertTrue(spreadsheet_importer.SpreadsheetImporter._looks_like_date('2000-04-30'))
    ...
    def test_validate_data(self):
        ...
        self.assertEqual(expected, spreadsheet_importer.SpreadsheetImporter._validate_data(self.db, data, dropbox_dir))
        ...
        expected = ['Replicate subject_id,site_id,lab_id,isolate_number,sequence_replicate_number p1,s1,l1,42,43 found 2 times in spreadsheet']
        self.assertEqual(expected, spreadsheet_importer.SpreadsheetImporter._validate_data(self.db, data, dropbox_dir))
        del data[1]
        ...
        self.assertEqual([], spreadsheet_importer.SpreadsheetImporter._validate_data(self.db, data, dropbox_dir))
        self.db.add_one_seqrep(data[0])
        expected = ['Replicate already found for subject_id,site_id,lab_id,isolate_number,sequence_replicate_number: p1,s1,l1,42,43']
        self.assertEqual(expected, spreadsheet_importer.SpreadsheetImporter._validate_data(self.db, data, dropbox_dir))
        ...
        expected = ['Subject(p1) + site(s1) + lab(l1) found more than once in database. Something very wrong!'] + expected
        self.assertEqual(expected, spreadsheet_importer.SpreadsheetImporter._validate_data(self.db, data, dropbox_dir))
    ...
    def test_archive_spreadsheet(self):
        ...
        got_filename = spreadsheet_importer.SpreadsheetImporter._archive_spreadsheet(xlsx_file, archive_dir)
        ...
        got_filename = spreadsheet_importer.SpreadsheetImporter._archive_spreadsheet(xlsx_file2, archive_dir)
    ...
    def test_import_reads_and_update_db(self):
        ...
        importer = spreadsheet_importer.SpreadsheetImporter(dropbox_dir, xlsx_file, db_ini_file, archive_dir, tsv_file, db_backup_dir=db_backup_dir)
        ...
        expected_tsv_lines = [
            '\t'.join(['seqrep_id', 'sample_id', 'isolate_id', 'sequence_replicate_number', 'reads1', 'reads2', 'reads1_md5', 'reads2_md5']),
            '\t'.join(['1', '1', '1', '43', reads_prefix + '.1_1.fq', reads_prefix + '.1_2.fq', 'abcdefghijklmnopqrstuvwyx123456', 'abcdefghijklmnopqrstuvwyx123457']),
            '\t'.join(['2', '2', '2', '45', reads_prefix + '.2_1.fq', reads_prefix + '.2_2.fq', 'a73817805eb1d44ca88eb5cb794c7de7', 'd468360c689d482b227256d887a05996']),
        ]
    ...
    def test_run(self):
        ...
        importer = spreadsheet_importer.SpreadsheetImporter(dropbox_dir, xlsx_file, db_ini_file, archive_dir, tsv_file)
