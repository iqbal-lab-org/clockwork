...
class TestReadMap(unittest.TestCase):
    def test_map_reads(self):
        ...
        if os.path.exists(tmp_sam):
            os.unlink(tmp_sam)
        read_map.map_reads(ref_fasta, reads1, reads2, tmp_sam, read_group=('1','GROUP_NAME'))
    ...
    def test_map_reads_markdup_and_rmdup(self):
        ...
        with self.assertRaises(read_map.Error):
            read_map.map_reads('ref_fasta', 'reads1', 'reads2', 'sam', rmdup=True, markdup=True)


    def test_map_reads_set(self):
        '''test map_reads_set'''
        reads1 = [os.path.join(data_dir, 'reads_set_' + str(i) + '.1.fq') for i in range(1,4,1)]
        reads2 = [os.path.join(data_dir, 'reads_set_' + str(i) + '.2.fq') for i in range(1,4,1)]
        ref_fasta = os.path.join(data_dir, 'ref.fa')
        try:
            os.unlink(tmp_bam)
        except:
        ...
        read_map.map_reads_set(ref_fasta, reads1, reads2, tmp_bam, rmdup=True, read_group=('1', 'GROUP_NAME'))
