...
class TestDb(unittest.TestCase):
    ...
    def test_add_row_to_table(self):
        ...
        db_schema.tables['Test'] = [('col1', 'integer'), ('col2', 'integer'), ('col3', 'text')]
    ...
    def test_update_row(self):
        ...
        self.db.update_row('Test', {'col1': 2, 'col2': 3}, {'col2': 100, 'col3': 'shrubbery'})
        rows[1]['col2'] = 100
    ...
    def test_has_sample_isolate_seq_replicate(self):
        ...
        self.assertFalse(self.db.has_sample_isolate_seq_replicate(41, 'lab_isolate_id', 2))
        self.assertFalse(self.db.has_sample_isolate_seq_replicate(42, 'lab_isolate_id', 2))
        self.assertFalse(self.db.has_sample_isolate_seq_replicate(42, 'wrong_lab_isolate_id', 3))
        self.assertTrue(self.db.has_sample_isolate_seq_replicate(42, 'lab_isolate_id', 3))
    ...
    def test_get_sample_and_replicate_uniqueness(self):
        ...
        self.assertEqual((True, False, None), self.db._get_sample_and_replicate_uniqueness(data))
        self.db.add_one_seqrep(data)
        self.assertEqual((True, True, 1), self.db._get_sample_and_replicate_uniqueness(data))

        data['isolate_number'] = '41'
        self.assertEqual((True, False, 1), self.db._get_sample_and_replicate_uniqueness(data))
        ...
        data['subject_id'] = 'p2'
        self.db.add_one_seqrep(data)
        self.assertEqual((True, True, 2), self.db._get_sample_and_replicate_uniqueness(data))
    ...
    def test_make_remove_contam_jobs_tsv(self):
        ...
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.2.1'}, {'import_status': 1, 'withdrawn': 1})
        ...
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.3.1'}, {'import_status': 1})
        pipeline_row3 = {'isolate_id': 3, 'seqrep_id': 3, 'seqrep_pool': None, 'version': '1.0.0',
        ...
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.4.1'}, {'import_status': 1})
        pipeline_row4 = {'isolate_id': 4, 'seqrep_id': 4, 'seqrep_pool': None, 'version': '1.0.1',
            'pipeline_name': 'remove_contam', 'status': 0, 'reference_id': 1}
        ...
        refs_root = '/refs/'
        refdir = reference_dir.ReferenceDir(pipeline_references_root_dir=refs_root, reference_id=1)

        expected_tsv_header = '\t'.join(['reads_in1', 'reads_in2', 'counts_tsv', 'reads_contam1',
            'reads_contam2', 'reads_remove_contam1', 'reads_remove_contam2',
            'sample_id', 'seqrep_id', 'isolate_id', 'sequence_replicate_number', 'reference_id',
            'ref_fasta', 'contam_tsv'])
    ...
    def test_make_qc_jobs_tsv(self):
        ...
        self.db.add_one_seqrep(data_dict2)
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.2.1'}, {'import_status': 1, 'withdrawn': 1})
        pipeline_row2 = {'isolate_id': 2, 'seqrep_id': 2, 'seqrep_pool': None, 'version': '1.0.0',
        ...
        self.db.add_one_seqrep(data_dict5)
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.5.1'}, {'import_status': 1})
        pipeline_row5 = {'isolate_id': 5, 'seqrep_id': 5, 'seqrep_pool': None, 'version': '1.0.0',
            'pipeline_name': 'remove_contam', 'status': 1, 'reference_id': 1}
        ...
        self.db.add_reference('ref_name')
        refdir = reference_dir.ReferenceDir(pipeline_references_root_dir=refs_root, reference_id=1)
        expected_tsv_header = '\t'.join(['reads_in1', 'reads_in2', 'output_dir', 'sample_id', 'seqrep_id',
            'isolate_id', 'sequence_replicate_number', 'reference_id', 'ref_fasta'])
        ...
        self.db.make_qc_jobs_tsv(tmp_out, pipeline_root, 1, refs_root, pipeline_version='1.0.0')
        expected_lines = [expected_tsv_header, expected_tsv_6_0]
        self.check_tsv(expected_lines, tmp_out)
        os.unlink(tmp_out)
        expected_rows.append({'isolate_id': 6, 'seqrep_id': 6, 'seqrep_pool': None, 'version': '1.0.0',
            'pipeline_name': 'qc', 'status': 0, 'reference_id': 1})
        expected_rows.sort(key=itemgetter('seqrep_id'))
        got_rows = self.db.get_rows_from_table('Pipeline')
        got_rows.sort(key=itemgetter('seqrep_id'))
        self.assertEqual(expected_rows, got_rows)
        ...
        self.db.make_qc_jobs_tsv(tmp_out, pipeline_root, 1, refs_root, pipeline_version='1.0.1', dataset_name='group2')
        expected_lines = [expected_tsv_header, expected_tsv_6_1]
        self.check_tsv(expected_lines, tmp_out)
        os.unlink(tmp_out)
        expected_rows.append({'isolate_id': 6, 'seqrep_id': 6, 'seqrep_pool': None, 'version': '1.0.1',
            'pipeline_name': 'qc', 'status': 0, 'reference_id': 1})
        expected_rows.sort(key=itemgetter('seqrep_id'))
        got_rows = self.db.get_rows_from_table('Pipeline')
        got_rows.sort(key=itemgetter('seqrep_id'))
    ...
    def test_make_variant_call_jobs_tsv(self):
        ...
        data_dict3 = make_data_dict(3)
        self.db.add_one_seqrep(data_dict3)
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.3.1'}, {'import_status': 1, 'withdrawn': 0})
        data_dict4 = make_data_dict(4)
        self.db.add_one_seqrep(data_dict4)
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.4.1'}, {'import_status': 1, 'withdrawn': 0})
        pipeline_row4 = {'isolate_id': 4, 'seqrep_id': 4, 'seqrep_pool': None, 'version': '1.0.0',
            'pipeline_name': 'remove_contam', 'status': 0, 'reference_id': 1}
        self.db.add_row_to_table('Pipeline', pipeline_row4)
        ...
        self.db.add_one_seqrep(data_dict5)
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.5.1'}, {'import_status': 1, 'withdrawn': 0})
        pipeline_row5 = {'isolate_id': 5, 'seqrep_id': 5, 'seqrep_pool': None, 'version': '1.0.0',
            'pipeline_name': 'remove_contam', 'status': 1, 'reference_id': 1}
        self.db.add_row_to_table('Pipeline', pipeline_row5)
        pipeline_row5.update({'pipeline_name': 'variant_call', 'seqrep_id': None, 'seqrep_pool': '1'})
        self.db.add_row_to_table('Pipeline', pipeline_row5)
        ...
        self.db.add_one_seqrep(data_dict6)
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.6.1'}, {'import_status': 1, 'withdrawn': 0})
        pipeline_row6 = {'isolate_id': 6, 'seqrep_id': 6, 'seqrep_pool': None, 'version': '1.0.1',
            'pipeline_name': 'remove_contam', 'status': 1, 'reference_id': 1}
        self.db.add_row_to_table('Pipeline', pipeline_row6)
        pipeline_row6['pipeline_name'] = 'variant_call'
        self.db.add_row_to_table('Pipeline', pipeline_row6)
        self.db.update_row('Isolate', {'isolate_id': 6}, {'pool_sequence_replicates': 0})
        ...
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.7.1'}, {'import_status': 1, 'withdrawn': 0})
        pipeline_row7 = {'isolate_id': 7, 'seqrep_id': 7, 'seqrep_pool': None, 'version': '1.0.0',
            'pipeline_name': 'remove_contam', 'status': 1, 'reference_id': 1}
        ...
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.8.1'}, {'import_status': 1, 'withdrawn': 0})
        pipeline_row8 = {'isolate_id': 7, 'seqrep_id': 8, 'seqrep_pool': None, 'version': '1.0.0',
            'pipeline_name': 'remove_contam', 'status': 1, 'reference_id': 1}
        self.db.add_row_to_table('Pipeline', pipeline_row8)
        self.db.update_row('Isolate', {'isolate_id': 7}, {'pool_sequence_replicates': 0})
        ...
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.9.1'}, {'import_status': 1, 'withdrawn': 0})
        pipeline_row9 = {'isolate_id': 9, 'seqrep_id': 9, 'seqrep_pool': None, 'version': '1.0.0',
            'pipeline_name': 'remove_contam', 'status': 1, 'reference_id': 1}
        ...
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.10.1'}, {'import_status': 1, 'withdrawn': 0})
        pipeline_row10 = {'isolate_id': 9, 'seqrep_id': 10, 'seqrep_pool': None, 'version': '1.0.0',
            'pipeline_name': 'remove_contam', 'status': 1, 'reference_id': 1}
        ...
        refs_root = '/refs/'
        refdir1 = reference_dir.ReferenceDir(pipeline_references_root_dir=refs_root, reference_id=1)
        refdir2 = reference_dir.ReferenceDir(pipeline_references_root_dir=refs_root, reference_id=2)
        ...
        expected_rows.extend([
            {'isolate_id': 6, 'seqrep_id': 6, 'seqrep_pool': None, 'version': '1.0.0',
                'pipeline_name': 'variant_call', 'status': 0, 'reference_id': 1},
            {'isolate_id': 7, 'seqrep_id': 7, 'seqrep_pool': None, 'version': '1.0.0',
                'pipeline_name': 'variant_call', 'status': 0, 'reference_id': 1},
            {'isolate_id': 7, 'seqrep_id': 8, 'seqrep_pool': None, 'version': '1.0.0',
                'pipeline_name': 'variant_call', 'status': 0, 'reference_id': 1},
            {'isolate_id': 8, 'seqrep_id': None, 'seqrep_pool': '1_2', 'version': '1.0.0',
                'pipeline_name': 'variant_call', 'status': 0, 'reference_id': 1},
        ])


        def sort_pipeline_rows(rows):
            rows.sort(key=lambda x: (x['isolate_id'], -1 if x['seqrep_id'] is None else x['seqrep_id'], -1 if x['seqrep_pool'] is None else x['seqrep_pool']))
        ...
        os.unlink(tmp_out)
        expected_rows.extend([
            {'isolate_id': 5, 'seqrep_id': None, 'seqrep_pool': '1', 'version': '1.0.1',
                'pipeline_name': 'variant_call', 'status': 0, 'reference_id': 1},
            {'isolate_id': 7, 'seqrep_id': 7, 'seqrep_pool': None, 'version': '1.0.1',
                'pipeline_name': 'variant_call', 'status': 0, 'reference_id': 1},
            {'isolate_id': 7, 'seqrep_id': 8, 'seqrep_pool': None, 'version': '1.0.1',
                'pipeline_name': 'variant_call', 'status': 0, 'reference_id': 1},
            {'isolate_id': 8, 'seqrep_id': None, 'seqrep_pool': '1_2', 'version': '1.0.1',
                'pipeline_name': 'variant_call', 'status': 0, 'reference_id': 1},
        ])
        ...
        self.check_tsv(expected_lines, tmp_out)
        os.unlink(tmp_out)
        expected_rows.append({'isolate_id': 8, 'seqrep_id': None, 'seqrep_pool': '1_2', 'version': '1.0.1',
            'pipeline_name': 'variant_call', 'status': 0, 'reference_id': 2})
        sort_pipeline_rows(expected_rows)
        got_rows = self.db.get_rows_from_table('Pipeline')
        sort_pipeline_rows(got_rows)
        self.assertEqual(expected_rows, got_rows)
    ...
    def test_make_generic_pipeline_jobs_tsv(self):
        ...
        self.db.add_one_seqrep(data_dict7)
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.7.1'}, {'import_status': 1, 'withdrawn': 0})
        pipeline_row7 = {'isolate_id': 6, 'seqrep_id': 7, 'seqrep_pool': None, 'version': '1.0.0',
            'pipeline_name': 'remove_contam', 'status': 1, 'reference_id': 1}
        self.db.add_row_to_table('Pipeline', pipeline_row7)
        self.db.update_row('Isolate', {'isolate_id': 6}, {'pool_sequence_replicates': 0})
        ...
        self.db.add_one_seqrep(data_dict8)
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.8.1'}, {'import_status': 1, 'withdrawn': 0})
        pipeline_row8 = {'isolate_id': 8, 'seqrep_id': 8, 'seqrep_pool': None, 'version': '1.0.0',
            'pipeline_name': 'remove_contam', 'status': 1, 'reference_id': 1}
        self.db.add_row_to_table('Pipeline', pipeline_row8)
        ...
        self.db.add_one_seqrep(data_dict9)
        self.db.update_row('Seqrep', {'original_reads_file_1_md5': 'md5.9.1'}, {'import_status': 1, 'withdrawn': 0})
        pipeline_row9 = {'isolate_id': 8, 'seqrep_id': 9, 'seqrep_pool': None, 'version': '1.0.0',
            'pipeline_name': 'remove_contam', 'status': 1, 'reference_id': 1}
        self.db.add_row_to_table('Pipeline', pipeline_row9)
        ...
        def make_expected_tsv_line(pipeline_root, i, pool, seqrep_ids, sequence_replicate_numbers, pipeline_version):
            iso_dir = isolate_dir.IsolateDir(pipeline_root, i, i)
            sequence_replicate_numbers_string = '_'.join([str(x) for x in sequence_replicate_numbers])
            return '\t'.join([
                ' '.join([iso_dir.reads_filename('remove_contam', j, 1) for j in sequence_replicate_numbers]),
                ' '.join([iso_dir.reads_filename('remove_contam', j, 2) for j in sequence_replicate_numbers]),
                iso_dir.pipeline_dir(sequence_replicate_numbers_string, pipeline_name, pipeline_version),
                str(i),
                pool,
                str(i),
                '_'.join([str(x) for x in seqrep_ids]),
        ...
        expected_tsv_5 = make_expected_tsv_line(pipeline_root, 5, '1', [5], [1], '1.0.0')
        expected_tsv_6_1 = make_expected_tsv_line(pipeline_root, 6, '0', [6], [1], '1.0.0')
        expected_tsv_6_2 = make_expected_tsv_line(pipeline_root, 6, '0', [7], [2], '1.0.0')
        expected_tsv_7_pool_1_0_0 = make_expected_tsv_line(pipeline_root, 7, '1', [8,9], [1,2], '1.0.0')
        expected_tsv_7_pool_1_0_1 = make_expected_tsv_line(pipeline_root, 7, '1', [8,9], [1,2], '1.0.1')

        tmp_out = 'tmp.db_test.make_generic_pipeline_jobs_tsv.tsv'
        self.db.make_generic_pipeline_jobs_tsv(tmp_out, pipeline_root, pipeline_name, pipeline_version='1.0.0')
        expected_lines = [expected_tsv_header, expected_tsv_5, expected_tsv_6_1, expected_tsv_6_2, expected_tsv_7_pool_1_0_0]
        self.check_tsv(expected_lines, tmp_out)
        os.unlink(tmp_out)
        ...
        # are now "running")
        expected_rows.extend([
            {'isolate_id': 5, 'seqrep_id': None, 'seqrep_pool': '1', 'version': '1.0.0',
                'pipeline_name': pipeline_name, 'status': 0, 'reference_id': None},
            {'isolate_id': 6, 'seqrep_id': 6, 'seqrep_pool': None, 'version': '1.0.0',
                'pipeline_name': pipeline_name, 'status': 0, 'reference_id': None},
            {'isolate_id': 6, 'seqrep_id': 7, 'seqrep_pool': None, 'version': '1.0.0',
                'pipeline_name': pipeline_name, 'status': 0, 'reference_id': None},
            {'isolate_id': 7, 'seqrep_id': None, 'seqrep_pool': '1_2', 'version': '1.0.0',
                'pipeline_name': pipeline_name, 'status': 0, 'reference_id': None},
        ])
        ...
        self.db.make_generic_pipeline_jobs_tsv(tmp_out, pipeline_root, pipeline_name, pipeline_version='1.0.1', dataset_name='group2')
        expected_lines = [expected_tsv_header, expected_tsv_7_pool_1_0_1]
        self.check_tsv(expected_lines, tmp_out)
        os.unlink(tmp_out)
        expected_rows.extend([
            {'isolate_id': 7, 'seqrep_id': None, 'seqrep_pool': '1_2', 'version': '1.0.1',
                'pipeline_name': pipeline_name, 'status': 0, 'reference_id': None},
        ])
    ...
    def test_get_rows_from_table(self):
        ...
        self.assertEqual([{'col1': 0, 'col2': 1}], self.db.get_rows_from_table('Test', columns='col1, col2'))

        rows.append({'col1': 1, 'col2': 0, 'col3': 'covfefe'})
        self.db.add_row_to_table('Test', rows[-1])
        rows_cols_1_2 = [{'col1': 0, 'col2': 1}, {'col1': 1, 'col2': 0}]
        self.assertEqual(rows_cols_1_2, self.db.get_rows_from_table('Test', columns='col1, col2'))
        rows_cols_1_2.reverse()
        self.assertEqual(rows_cols_1_2, self.db.get_rows_from_table('Test', columns='col1, col2', order_by='col2'))
        self.assertEqual([rows[1]], self.db.get_rows_from_table('Test', where='col1 = 1'))


    def test_isolate_id_to_sample_id(self):
        '''test isolate_id_to_sample_id'''
        self.assertEqual(None, self.db.isolate_id_to_sample_id(0))
        new_row = {'isolate_id': 42, 'sample_id': 43, 'isolate_number_from_lab': 'foo',
            'pool_sequence_replicates': 0, 'ena_experiment_accession': 'ENA123'}
    ...
    def test_update_remove_contam_stats(self):
        ...
        seqrep_row = {'seqrep_id': 1, 'isolate_id': 1, 'sequence_replicate_number' : 3,
            'original_reads_file_1_md5': 'md51', 'original_reads_file_2_md5': 'md52',
            'remove_contam_reads_file_1_md5': 'md53', 'remove_contam_reads_file_2_md5': 'md54',
            'withdrawn': 0, 'import_status': 1, 'instrument_model': 'Illumina HiSeq 2500',
            'submission_date': '20170101', 'submit_to_ena': 0, 'ena_run_accession': 0, 'ena_on_hold': 0}
        self.db.add_row_to_table('Seqrep', seqrep_row)
        self.db._update_remove_contam_stats(1, counts_file)
        got_rows = self.db.get_rows_from_table('Read_counts')
    ...
    def test_update_qc_stats(self):
        ...
        seqrep_row = {'seqrep_id': 1, 'isolate_id': 2, 'sequence_replicate_number' : 3,
            'original_reads_file_1_md5': 'md51', 'original_reads_file_2_md5': 'md52',
            'remove_contam_reads_file_1_md5': 'md53', 'remove_contam_reads_file_2_md5': 'md54',
            'withdrawn': 0, 'import_status': 1, 'instrument_model': 'Illumina HiSeq 2500',
            'submission_date': '20170101', 'submit_to_ena': 0, 'ena_run_accession': 0, 'ena_on_hold': 0}
        ...
        self.db.add_row_to_table('Seqrep', seqrep_row)
        isolate_row = {'isolate_id': 2, 'sample_id': 1, 'isolate_number_from_lab': 'i1',
            'pool_sequence_replicates': 1, 'ena_experiment_accession': None}
        self.db.add_row_to_table('Isolate', isolate_row)
    ...
    def test_update_finished_pipeline_run(self):
        ...
        pipeline_row = {'isolate_id': 1, 'seqrep_id': 1, 'seqrep_pool': None,  'version': clockwork_version,
                'pipeline_name':'pipeline_name', 'status': 1, 'reference_id': 1}
        self.db.add_row_to_table('Pipeline', pipeline_row)
        with self.assertRaises(db.Error):
            self.db.update_finished_pipeline_run(1, 1, None, 'pipeline_name', 1, pipeline_version=clockwork_version)
        ...
        self.db.add_row_to_table('Pipeline', pipeline_row)
        with self.assertRaises(db.Error):
            self.db.update_finished_pipeline_run(1, 2, None, 'pipeline_name', 1, pipeline_version=clockwork_version)

        # Should run with no errors
        pipeline_row['seqrep_id'] = 3
        pipeline_row['status'] = 0
        self.db.add_row_to_table('Pipeline', pipeline_row)
        self.db.update_finished_pipeline_run(1, 3, None, 'pipeline_name', 1, pipeline_version=clockwork_version)
        expected_rows = [{'isolate_id': 1, 'seqrep_id': 3, 'seqrep_pool': None,
            'version': clockwork_version, 'pipeline_name': 'pipeline_name', 'status': 1, 'reference_id': 1}]
        ...
        self.db.add_row_to_table('Pipeline', pipeline_row)
        self.db.update_finished_pipeline_run(1, 4, None, 'pipeline_name', -1, pipeline_version=clockwork_version, reference_id=2)
        expected_rows = [{'isolate_id': 1, 'seqrep_id': 4, 'seqrep_pool': None,
            'version': clockwork_version, 'pipeline_name': 'pipeline_name', 'status': -1, 'reference_id': 2}]
        got_rows = self.db.get_rows_from_table('Pipeline', where='seqrep_id = 4')
        self.assertEqual(expected_rows, got_rows)

        # Test when pooled
        pipeline_row = {'isolate_id': 2, 'seqrep_id': None, 'seqrep_pool': '42_43',  'version': clockwork_version,
                'pipeline_name':'pipeline_name', 'status': 0, 'reference_id': 1}
        self.db.add_row_to_table('Pipeline', pipeline_row)
        self.db.update_finished_pipeline_run(2, None, '42_43', 'pipeline_name', 1, pipeline_version=clockwork_version, reference_id=1)
        expected_rows = [{'isolate_id': 2, 'seqrep_id': None, 'seqrep_pool': '42_43',
            'version': clockwork_version, 'pipeline_name': 'pipeline_name', 'status': 1, 'reference_id': 1}]
    ...
    def test_update_finished_pipeline_run_remove_contam(self):
        ...
        pipeline_row = {'isolate_id': isolate_id, 'seqrep_id': seqrep_id, 'seqrep_pool': None,  'version': clockwork_version,
                'pipeline_name':'remove_contam', 'status': 0, 'reference_id': 1}
        self.db.add_row_to_table('Pipeline', pipeline_row)

        to_copy_counts_file = os.path.join(data_dir, 'update_finished_pipeline_run_remove_contam', 'counts.tsv')
        ...
        self.db.update_finished_pipeline_run(isolate_id, seqrep_id, None, 'remove_contam', 1, reference_id=1, pipeline_version=clockwork_version, pipeline_root=pipeline_root)
        got_rows = self.db.get_rows_from_table('Seqrep', where='seqrep_id =' + str(seqrep_id))
        self.assertEqual(1, len(got_rows))
    ...
    def test_load_success_jobs_file(self):
        ...
        got = db.Db._load_success_jobs_file(os.path.join(data_dir, 'load_success_jobs_file.no_pool_column.tsv'))
        self.assertEqual(expected, got)

        expected = {(1, '3', '1', False), (2, '4_5', '42_43', True), (3, '6', '2', True)}
        got = db.Db._load_success_jobs_file(os.path.join(data_dir, 'load_success_jobs_file.with_pool_column.tsv'))
        self.assertEqual(expected, got)
    ...
    def test_update_finished_pipeline_run_failed_jobs_not_pooled(self):
        ...
        for i in [1,2,3]:
            pipeline_row = {'isolate_id': i, 'seqrep_id': i, 'seqrep_pool': None, 'version': clockwork_version,
                'pipeline_name': 'pipeline_name', 'status': 0, 'reference_id': 1}
            self.db.add_row_to_table('Pipeline', pipeline_row)

        success_tsv = os.path.join(data_dir, 'update_finished_pipeline_run_failed_jobs.success.tsv')
        jobs_tsv = os.path.join(data_dir, 'update_finished_pipeline_run_failed_jobs.jobs.tsv')
        self.db.update_finished_pipeline_run_failed_jobs(jobs_tsv, success_tsv, 'pipeline_name')
        got_rows = self.db.get_rows_from_table('Pipeline')
        got_rows.sort(key=itemgetter('seqrep_id'))
        expected_rows = [
            {'isolate_id': 1, 'seqrep_id': 1, 'seqrep_pool': None, 'version': clockwork_version,
                'pipeline_name': 'pipeline_name', 'status': 0, 'reference_id': 1},
            {'isolate_id': 2, 'seqrep_id': 2, 'seqrep_pool': None, 'version': clockwork_version,
                'pipeline_name': 'pipeline_name', 'status': -1, 'reference_id': 1},
            {'isolate_id': 3, 'seqrep_id': 3, 'seqrep_pool': None, 'version': clockwork_version,
                'pipeline_name': 'pipeline_name', 'status': -1, 'reference_id': 1},
        ]
        self.assertEqual(expected_rows, got_rows)
    ...
    def test_update_finished_pipeline_run_failed_jobs_pooled(self):
        ...
        pipeline_rows = [
            {'isolate_id': 1, 'seqrep_id': 1, 'seqrep_pool': None, 'version': clockwork_version,
                'pipeline_name': 'pipeline_name', 'status': 0, 'reference_id': 1},
            {'isolate_id': 1, 'seqrep_id': 2, 'seqrep_pool': None, 'version': clockwork_version,
                'pipeline_name': 'pipeline_name', 'status': 0, 'reference_id': 1},
            {'isolate_id': 2, 'seqrep_id': None, 'seqrep_pool': '1', 'version': clockwork_version,
                'pipeline_name': 'pipeline_name', 'status': 0, 'reference_id': 1},
            {'isolate_id': 3, 'seqrep_id': None, 'seqrep_pool': '42_43', 'version': clockwork_version,
                'pipeline_name': 'pipeline_name', 'status': 0, 'reference_id': 1},
        ]
        ...
        success_tsv = os.path.join(data_dir, 'update_finished_pipeline_run_failed_jobs_pooled.success.tsv')
        jobs_tsv = os.path.join(data_dir, 'update_finished_pipeline_run_failed_jobs_pooled.jobs.tsv')
        self.db.update_finished_pipeline_run_failed_jobs(jobs_tsv, success_tsv, 'pipeline_name', reference_id=1)
        got_rows = self.db.get_rows_from_table('Pipeline')
    ...
    def test_get_reference_dir(self):
        ...
        expected = reference_dir.ReferenceDir(pipeline_references_root_dir=root, reference_id=1)
        self.assertEqual(expected, got)
