...
class TestReferenceDir(unittest.TestCase):
    ...
    def test_add_remove_contam_metadata_tsv(self):
        ...
        bad_files = [
            os.path.join(
                data_dir, "add_remove_contam_metadata_tsv.ref.missing_from_tsv.tsv"
            ),
            os.path.join(
                data_dir, "add_remove_contam_metadata_tsv.ref.extra_in_tsv.tsv"
            ),
        ]
        ...
        ref_dir.add_remove_contam_metadata_tsv(
            os.path.join(data_dir, "add_remove_contam_metadata_tsv.ref.good.tsv")
        )
        shutil.rmtree(tmp_root_dir)
