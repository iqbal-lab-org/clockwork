...
fastaq to_perfect_reads --seed 46 $tmp_ref_dir/contam.fa - 200 1 3 75 | fastaq deinterleave - tmp.reads.1.contam.1.fq tmp.reads.1.contam.2.fq
fastaq to_perfect_reads --seed 47 $tmp_ref_dir/other.fa - 200 1 2 75 | fastaq deinterleave - tmp.reads.1.other.1.fq tmp.reads.1.other.2.fq
...
fastaq to_perfect_reads --seed 48 $tmp_ref_dir/ref.fa - 200 1 10 75 | fastaq deinterleave - tmp.reads.3.ref.1.fq tmp.reads.3.ref.2.fq
fastaq to_perfect_reads --seed 49 $tmp_ref_dir/contam.fa - 200 1 1 75 | fastaq deinterleave - tmp.reads.3.contam.1.fq tmp.reads.3.contam.2.fq
fastaq to_perfect_reads --seed 50 $tmp_ref_dir/other.fa - 200 1 1 75 | fastaq deinterleave - tmp.reads.3.other.1.fq tmp.reads.3.other.2.fq

fastaq to_perfect_reads --seed 51 $tmp_ref_dir/ref.fa - 200 1 10 75 | fastaq deinterleave - tmp.reads.4.ref.1.fq tmp.reads.4.ref.2.fq
...
fastaq to_perfect_reads --seed 53 $tmp_ref_dir/other.fa - 200 1 1 75 | fastaq deinterleave - tmp.reads.4.other.1.fq tmp.reads.4.other.2.fq
...
p3,s1,l1,46,47,20171225,$dropbox/reads.3.1.fq.gz,0,$dropbox/reads.3.2.fq.gz,0,g1,Illumina HiSeq 2000,Center 1,0,0,0,0
p4,s1,l1,48,49,20171225,$dropbox/reads.4.1.fq.gz,0,$dropbox/reads.4.2.fq.gz,0,g2,Illumina HiSeq 2000,Center 1,0,0,0,0" | sed 's/,/\t/g' > $import_tsv


cat $tmp_ref_dir/ref.fa $tmp_ref_dir/contam.fa > $tmp_ref_dir/remove_contam.ref.fa
echo "ref	0	ref.1
contam	1	contam.1" > $tmp_ref_dir/remove_contam.ref.metadata.tsv

clockwork make_empty_db $db_pipeline_config
clockwork reference_prepare --db_config_file $db_pipeline_config --pipeline_references_root $pipeline_ref_dir --contam_tsv $tmp_ref_dir/remove_contam.ref.metadata.tsv --name remove_contam_ref $tmp_ref_dir/remove_contam.ref.fa
clockwork reference_prepare --contam_tsv $tmp_ref_dir/remove_contam.ref.metadata.tsv --outdir $ref_dir $tmp_ref_dir/remove_contam.ref.fa
rm -rf $tmp_ref_dir
...
rm $pipeline_root/00/00/00/03/3/Reads/reads.original.47.1.fq.gz
rm $pipeline_root/00/00/00/03/3/Reads/reads.original.47.2.fq.gz
echo "whoops" | gzip -c > $pipeline_root/00/00/00/03/3/Reads/reads.original.47.1.fq.gz
echo "covfefe" | gzip -c > $pipeline_root/00/00/00/03/3/Reads/reads.original.47.2.fq.gz
