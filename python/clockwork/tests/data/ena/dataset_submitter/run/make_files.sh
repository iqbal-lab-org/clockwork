...
# which are used to test the method DatasetSubmitter._get_data_from_db
...
db_pipeline_config=$PWD/../config.ini
mysql_cnf=db.cnf
...
for i in {1..7}; do
    fastaq to_perfect_reads --seed 45 $tmp_ref_dir/ref.fa - 200 1 10 75 | fastaq deinterleave - tmp.reads.$i.ref.1.fq tmp.reads.$i.ref.2.fq
    fastaq to_perfect_reads --seed 46 $tmp_ref_dir/contam.fa - 200 1 1 75 | fastaq deinterleave - tmp.reads.$i.contam.1.fq tmp.reads.$i.contam.2.fq
    fastaq to_perfect_reads --seed 47 $tmp_ref_dir/other.fa - 200 1 1 75 | fastaq deinterleave - tmp.reads.$i.other.1.fq tmp.reads.$i.other.2.fq
done

for i in {1..7}; do for j in 1 2; do cat tmp.reads.$i.*.$j.fq | gzip -c > $dropbox/reads.$i.$j.fq.gz; done; done
rm tmp.reads.*
...
for i in {1..7}; do for j in 1 2; do md5sum $dropbox/reads.$i.$j.fq.gz > $dropbox/reads.$i.$j.fq.gz.md5; done; done
...
p1,s1,l1,42,43,20171225,$dropbox/reads.1.1.fq.gz,0,$dropbox/reads.1.2.fq.gz,0,g1,Illumina HiSeq 2500,01,1,1,0,0
p2,s1,l1,44,1,20171225,$dropbox/reads.2.1.fq.gz,0,$dropbox/reads.2.2.fq.gz,0,g1,Illumina HiSeq 2500,01,1,1,0,0
p2,s1,l1,44,2,20171225,$dropbox/reads.3.1.fq.gz,0,$dropbox/reads.3.2.fq.gz,0,g1,Illumina HiSeq 2500,01,1,1,0,0
p2,s1,l1,45,1,20171225,$dropbox/reads.4.1.fq.gz,0,$dropbox/reads.4.2.fq.gz,0,g1,Illumina HiSeq 2500,01,1,1,0,0
p3,s1,l2,46,1,20171225,$dropbox/reads.5.1.fq.gz,0,$dropbox/reads.5.2.fq.gz,0,g1,Illumina HiSeq 2500,01,1,1,0,0
p4,s1,l2,47,1,20171225,$dropbox/reads.6.1.fq.gz,0,$dropbox/reads.6.2.fq.gz,0,g1,Illumina HiSeq 2500,01,0,0,0,0
p5,s2,l1,48,49,20171225,$dropbox/reads.7.1.fq.gz,0,$dropbox/reads.7.2.fq.gz,0,g2,Illumina HiSeq 2500,01,1,1,0,0" | sed 's/,/\t/g' > $import_tsv
...
rm $pipeline_root/00/00/00/05/6/Reads/reads.remove_contam.49.1.fq.gz
rm $pipeline_root/00/00/00/05/6/Reads/reads.remove_contam.49.2.fq.gz
echo "whoops" | gzip -c > $pipeline_root/00/00/00/05/6/Reads/reads.remove_contam.49.1.fq.gz
echo "covfefe" | gzip -c > $pipeline_root/00/00/00/05/6/Reads/reads.remove_contam.49.2.fq.gz
