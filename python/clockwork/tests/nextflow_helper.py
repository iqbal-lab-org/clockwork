...
clockwork_python_root = os.path.abspath(
    os.path.join(clockwork_python_modules_dir, os.pardir)
)
...
def write_config_file():
    with open(config_file, "w") as f:
        print(
            'env.PYTHONPATH = "', clockwork_python_root, ':$PYTHONPATH"', sep="", file=f
        )
        print(
            'env.PATH = "',
            os.path.join(clockwork_scripts_dir),
            ':$PATH"',
            sep="",
            file=f,
        )
