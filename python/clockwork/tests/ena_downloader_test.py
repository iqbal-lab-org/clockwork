...
class TestEnaDownloader(unittest.TestCase):
    def test_load_data_infile(self):
        ...
        infile_repeat_sample_name = os.path.join(data_dir, 'load_data_infile.repeat_sample_name.tsv')
        with self.assertRaises(ena_downloader.Error):
            ena_downloader.EnaDownloader._load_data_infile(infile_repeat_sample_name)

        infile_repeat_accession = os.path.join(data_dir, 'load_data_infile.repeat_accesion.tsv')
        with self.assertRaises(ena_downloader.Error):
    ...
    def test_rename_files(self):
        ...
        input_data = ena_downloader.EnaDownloader._load_data_infile(os.path.join(data_dir, 'rename_files.data.tsv'))
        ...
        expected_file_list = sorted([x + '_' + str(i) + '.fastq.gz' for i in (1,2) for x in runs])
    ...
    def test_ena_run_to_sample_and_instrument_model(self):
        ...
        got_sample, got_instrument, got_center_name = ena_downloader.EnaDownloader._ena_run_to_sample_and_instrument_model(run_accession)
    ...
    def test_write_import_tsv(self):
        ...
        ena_downloader.EnaDownloader._write_import_tsv(outfile, input_data, runs, 'site1', 'lab1', '20170101', 'group1', test_ena_dict=test_ena_dict)
